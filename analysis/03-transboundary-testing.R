...
do_fit <- function(.sp, .survey) {
  ...
  if (.survey == "synoptic") {
    dat0 <- surveyjoin::get_data(.sp, regions = c("pbs", "nwfsc")) |>
      ...
      filter(survey_name %in% c("NWFSC.Combo", "SYN HS", "SYN QCS", "SYN WCHG", "SYN WCVI")) |>
      filter(year <= 2024) # 2025 missing from NWFSC
    ...
    dat <- dat0 |>
      ...
      mutate(survey_group = ifelse(grepl("syn", survey_name, ignore.case = TRUE), "pbs", "nwfsc"))
    ...
    mesh <- make_mesh(dat, c("X", "Y"), cutoff = 25)
