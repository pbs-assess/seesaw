...
run_scenario <- function(label, overlap_years) {
  ...
  has_overlap_year <- d |>
    distinct(year, survey_abbrev) |>
    count(year, name = "n_regions") |>
    summarise(any_overlap = any(n_regions > 1L)) |>
    pull(any_overlap)

  fixed_formula <- if (isTRUE(has_overlap_year)) {
    observed ~ factor(year) + factor(survey_abbrev)
  } else {
...
ggsave("figs/pcod-qcs-hs-retro-add-rw-year.pdf", width = 9, height = 6)
